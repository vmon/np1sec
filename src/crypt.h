...
class Cryptic {
 protected:
  AsymmetricKey ephemeral_key;
  PublicKey ephemeral_pub_key;
  PrivateKey ephemeral_prv_key;
 ...
 public:
  ...
  Cryptic(const Cryptic& rhs) {
    ephemeral_key = AsymmetricKey(copy_crypto_resource(rhs.ephemeral_key));
    ephemeral_pub_key = PublicKey(copy_crypto_resource(rhs.ephemeral_pub_key));
    ephemeral_prv_key = PrivateKey(copy_crypto_resource(rhs.ephemeral_prv_key));
...
class  LongTermIDKey {
 ...
 public:
  ...
  void generate() {
    try {
      // TODO - Change this so KeyPairs contain PublicKey and PrivateKey instances.
      AsymmetricKeyPair pair = generate_key_pair();
      key_pair.first = static_cast<np1secPrivateKey>(pair.private_key.unwrap());
      key_pair.second = static_cast<np1secPublicKey>(pair.public_key.unwrap());
      initiated = true;
  ...
  void set_key_pair(KeyPair user_key_pair) {
    initiated = true;
    key_pair.first = copy_crypto_resource(PrivateKey(user_key_pair.first));
    key_pair.second = copy_crypto_resource(PublicKey(user_key_pair.second));
  }
